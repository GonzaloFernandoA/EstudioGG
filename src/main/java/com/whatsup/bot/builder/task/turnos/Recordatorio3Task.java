...
@Component
public class Recordatorio3Task implements ITask {
    private static final Logger log = LoggerFactory.getLogger(Recordatorio3Task.class);
    ...
    public Boolean CanRun(String lastAction) {
        return lastAction.contains("RECORDATORIO_3_");
    }
    ...
    public String getMessage(ExecuteParameter parameter) {
        ...
        log.info("Recordatorio3Task getMessage " + parameter.getName() + " telefono " + parameter.getPhoneNumber());
        LogMensajes message = logMensajesService.get(parameter.getWaId());
        log.info("Recordatorio3Task getMessage " + "result " + parameter.getMessage() + " message:"+message.getId());
        SqsMessageTurno messageTurno = new SqsMessageTurno(message.getId(), "recordatorio3", parameter.getMessage());
